...
def nearest_neighbor_route(df):
    ...
    unvisited = unvisited.drop(current_location.name)

    while not unvisited.empty:
        current_coordinates = (current_location['latitude'], current_location['longitude'])
        distances = unvisited.apply(lambda row: geodesic(current_coordinates, (row['latitude'], row['longitude'])).km, axis=1)
...
optimized_route = nearest_neighbor_route(selected_stops)
...
st.markdown(f"### Showing Top {num_stops} Stops in **{selected_state}**")
if selected_categories:
    st.markdown(f"**Filtered by Categories:** {', '.join(selected_categories)}")

st.dataframe(selected_stops[['name', 'city', 'categories', 'weighted__score']])
